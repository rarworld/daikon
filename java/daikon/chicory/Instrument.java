...
//        	}
  ...
  /**
   * Transforms Throw instructions to first assign the Exception to a local
...
//    	return (null);
   ...
   */
  private /*@Nullable*/ InstructionList
  add_tryCatch_instrumentation (String fullClassName, MethodContext c) {
...
// are FULL_FAME we need to add our 'nonce' variable to the local table.
  ...
  create_method_info (ClassInfo class_info, MethodGen mgen) {
    ...
    if (shouldInclude)
      return new MethodInfo(class_info, mgen.getName(), arg_names, arg_type_strings, exit_locs, throw_locs, isIncluded);
    else
      return null;
  }
   ...
   */
  protected void
  fix_local_variable_table (MethodGen mg) {
    ...
    if (!mg.isStatic()) {
        ...
        new_lvg = mg.addLocalVariable (l.getName(), l.getType(), l.getIndex(),
                                       l.getStart(), l.getEnd());
        if (debug) {
            out.format("Added <this> %s%n", new_lvg.getIndex() + ": "
                       + new_lvg.getName() + ", " + new_lvg.getType());
        }
        loc_index = 1;
        offset = 1;
        first_local_index++;
    ...
    for (int ii = 0; ii < arg_types.length; ii++) {
        ...
        if (debug) {
            out.format("Added param  %s%n", new_lvg.getIndex() + ": "
                       + new_lvg.getName() + ", " + new_lvg.getType());
        }
      ...
      for (int ii = first_local_index; ii < locals.length; ii++) {
        ...
        if (debug) {
            out.format("Added local  %s%n", new_lvg.getIndex() + ": "
                       + new_lvg.getName() + ", " + new_lvg.getType());
        }
